    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <h3 class="text-lg font-bold mb-4">Tambah Data Penelitian</h3>
                <form action="{{ route('penelitian.store') }}" method="POST" class="mb-8">
                    @csrf
                            ...
                            <tr class="bg-white border-b">
                                ...
                                <td class="px-4 py-2">
                                    ...
                                    <div class="flex gap-2">
                                        <button type="button" onclick="openBuktiModal({{ $item->id }}, 'penelitian')" class="text-blue-600 hover:text-blue-900 bg-blue-100 px-2 py-1 rounded text-xs">Tambah Bukti</button>
                                        <a href="{{ route('penelitian.edit', $item->id) }}" class="inline-flex items-center px-2 py-1 bg-yellow-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-600 focus:bg-yellow-600 active:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                            Edit
                                        </a>
                                        <form action="{{ route('penelitian.destroy', $item->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
